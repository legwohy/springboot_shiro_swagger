...
public class UserController {
    ...
    public ModelAndView addUser(@ApiParam(value = "实体用户")@RequestBody BackUser backUser){
        ModelAndView mv = new ModelAndView("user/add");
        String userMobile = backUser.getUserMobile();
        BackUser user = backUserService.findUserByLoginName(userMobile);
        if(user != null){
            // 用户已经存在
        }else {
    ...
    public ModelAndView updateUser(@RequestBody BackUser backUser){
        ModelAndView mv = new ModelAndView("user/updateUser");
        boolean flag = backUserService.updateUserBySelective(backUser);
        if(flag){
